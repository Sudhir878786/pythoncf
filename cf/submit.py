...
def submit(file: str):
    ...
    url = f"https://pythoncf.com/contest/{c_id}/submit"
    ...
    if not resp.url.startswith(f"https://pythoncf.com/contest/{c_id}/my"):
        console.print("[bold red]ERROR: [/] Submission failed.")
        return

    r = clnt.session.get(f"https://pythoncf.com/contest/{c_id}/my")
    ...
    console.print(f"[bold green]SUBMITTED[/] [bold blue]https://pythoncf.com/contest/{c_id}/submission/{sub_id}[/]")
    live_text = f"""
...
Memory:     [bold white]{sub_mem.strip()}[/]
    ...
    sub_watcher.connect(f"wss://pubsub.pythoncf.com/ws/s_{pc}/s_{cc}?_={int(time.time())}")
