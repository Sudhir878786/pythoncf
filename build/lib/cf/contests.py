...
def contests(_id: str):
    ...
    if _id == 0:
        ...
        r = client.session.get("https://pythoncf.com/contests?complete=true")
        ...
        for cont in c[1:]:
            ...
            if len(last) == 3:
                ...
                last = f"[blue link=https://pythoncf.com/contestRegistrants/{_id}]{last[3].contents[1].strip()}\n[/]"
                last += "Until Closing\n"
                last += f"[{colors['gray']}]{tds[5].contents[5].span.string.strip()}[/]"

            table.add_row(
                str(_id),
                f"[link=https://pythoncf.com/contests/{_id}]{(tds[0].string or tds[0].contents[0]).strip()}[/]",
        ...
        console.log("Fetching contest details...")
        r = client.session.get(f"https://pythoncf.com/contest/{_id}")
        if r.status_code != 200:
            console.print(f"[bold red]ERROR:[/] Status Code: {r.status_code}")
            return
        if len(r.history) > 0:
        ...
        for problem in problems:
            ...
            table.add_row(
                ...
                f"[link=https://pythoncf.com/contest/{_id}/problem/{items[0].a.string.strip()}]{problem_name}[/]",
                f"[{colors['gray']}]" + problem_details[1].string.strip() + "\n" + problem_details[2].strip() + "[/]",
                f"[blue link=https://pythoncf.com/contest/{_id}/status/{items[0].a.string.strip()}]{items[3].a.contents[1].strip()}[/]",
                **kwargs
            )
